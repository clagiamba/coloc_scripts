...
##
#### !!!!! MUST USE THE LOG OF THE OR HERE!!! ?? ALSO IN OUR COLOC2?? not the same for CARDIoGRAMplusC4D_formatted, for now don't use this 
        if (type=="cc") {
                #dataset.biom = list(snp = merged.data$SNPID, beta = log(merged.data$BETA.biom), varbeta= (merged.data$SE.biom)^2,
                #dataset.biom = list(snp = merged.data$SNPID, beta = merged.data$BETA.biom, varbeta= (merged.data$SE.biom)^2,
                if (length(merged.data$BETA.biom[merged.data$BETA.biom<0])>0) log=FALSE else log=TRUE
                if (log) {
                    dataset.biom = list(snp = merged.data$SNPID, beta = log(merged.data$BETA.biom), varbeta= (merged.data$SE.biom)^2,
                                   s=merged.data$s1, type = type, MAF=merged.data$MAF.biom,N=merged.data$N.biom)
                    } else {
                    dataset.biom = list(snp = merged.data$SNPID, beta = merged.data$BETA.biom, varbeta= (merged.data$SE.biom)^2,
                                    s=merged.data$s1, type = type, MAF=merged.data$MAF.biom,N=merged.data$N.biom)
                    }
...
##     
         ...
         min.beta.biom <- merged.data[which.min(merged.data$PVAL.biom), "BETA.biom"]
         min.beta.eqtl <- merged.data[which.min(merged.data$PVAL.eqtl), "BETA.eqtl"]
         N.biom = max(merged.data$N.biom)
         N.eqtl = max(merged.data$N.eqtl)
         Ncases = NA 
         if (cc) Ncases = max(biom.df$Ncases)
